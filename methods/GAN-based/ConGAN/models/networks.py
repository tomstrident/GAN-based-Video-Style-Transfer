...
def define_F(init_type='normal', init_gain=0.02, gpu_ids=[]):
  net = FusionBlock()
  return init_net(net, init_type, init_gain, gpu_ids)
...
class FusionBlock(nn.Module):
    def __init__(self):
        ...
        reflection_pad = int(np.floor(3 / 2))
        conv1 = [nn.ReflectionPad2d(reflection_pad),
                 nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, stride=1, bias=True),
                 nn.InstanceNorm2d(num_features=32, affine=True),
                 nn.ReLU()]
                 
        conv2 = [nn.ReflectionPad2d(reflection_pad),
                 nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, bias=True),
                 nn.InstanceNorm2d(num_features=32, affine=True),
                 nn.ReLU()]
                 
        conv3 = [nn.ReflectionPad2d(reflection_pad),
                 nn.Conv2d(in_channels=32, out_channels=1, kernel_size=3, stride=1, bias=True),
                 nn.Sigmoid()]
                 
        sequence = conv1 + conv2 + conv3
        self.model = nn.Sequential(*sequence)

    def forward(self, simg, wimg):
        mask = self.model(simg - wimg)
        return mask*wimg + (1 - mask)*simg
