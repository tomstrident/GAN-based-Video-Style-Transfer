...
class SingleSintelVideo(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if index > 0:
      ...
      last_img =  self.transform(Image.open(last_fid))[:,:432,:]
      
    if index >= self.lt_len:
      past_fid = self.dataset[index - self.lt_len]
      past_img =  self.transform(Image.open(past_fid))[:,:432,:]
    
    return img[:,:432,:], last_img, past_img
...
def computeTCL(net, model, s_trg, img_fake, img1, img2):
  ...
  #print(img_fake.shape, img2.shape)
...
def evaluate_sintel(args, sintel_dir="D:/Datasets/MPI-Sintel-complete/"):
  ...
  out_path = "G:/Code/StarGANv2AdvCon/eval/"
  ...
  video_list = [os.path.join(sintel_dir, vid) for vid in train_list]
  video_list += [os.path.join(sintel_dir, vid) for vid in test_list]
