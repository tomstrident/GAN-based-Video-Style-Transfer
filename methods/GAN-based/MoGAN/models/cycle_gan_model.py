...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_MC', type=float, default=10.0, help='weight for motion cycle loss')
            parser.add_argument('--lambda_AM', type=float, default=1.0, help='weight for auxiliary motion loss')
            parser.add_argument('--lambda_MT', type=float, default=10.0, help='weight for motion translation loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'MC_A', 'AM_A', 'MT_A', 'MC_B', 'AM_B', 'MT_B']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        visual_names_A = ['real_A', 'fake_B', 'rec_A', 'real_A2', 'fake_B2', 'rec_A2', 'warp_B', 'mask_A']
        visual_names_B = ['real_B', 'fake_A', 'rec_B', 'real_B2', 'fake_A2', 'rec_B2', 'warp_A', 'mask_B']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netM_A = networks.define_G(2, 2, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netM_B = networks.define_G(2, 2, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.set_requires_grad([self.netM_A, self.netM_B], False)
    ...
    def forward_eval(self, inp, AtoB=True):
    ...
    def backward_G(self):
        ...
        lambda_MC = self.opt.lambda_MC
        lambda_MT = self.opt.lambda_MT
        # Identity loss
        if lambda_idt > 0:
        ...
        #print("loss_cycle_B", self.loss_cycle_B.requires_grad)
        
        # MC loss
        self.loss_MC_A = (self.mask_A*torch.abs(self.bf_rec_A - self.bf_real_A)).mean() * lambda_MC
        self.loss_MC_B = (self.mask_B*torch.abs(self.bf_rec_B - self.bf_real_B)).mean() * lambda_MC
        
        # MT loss
        self.loss_MT_A = (self.mask_A*torch.abs(self.warp_B - self.fake_B2)).mean() * lambda_MT
        self.loss_MT_B = (self.mask_B*torch.abs(self.warp_A - self.fake_A2)).mean() * lambda_MT
        
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_MC_A + self.loss_MC_B + self.loss_MT_A + self.loss_MT_B
        self.loss_G.backward()#retain_graph=True
        
    def backward_M(self):
      lambda_AM = self.opt.lambda_AM

      # AM loss
      self.loss_AM_A = torch.abs(self.bf_M_A - self.bf_fake_B).mean() * lambda_AM
      self.loss_AM_B = torch.abs(self.bf_M_B - self.bf_fake_A).mean() * lambda_AM

      self.loss_M = self.loss_AM_A + self.loss_AM_B
